...
```
```{r}
packages <- c("data.table", "collapse", "bench")
...
kw_fast <- function(x, g){
     if (length(x) != length(g) | length(x) == 0)
            stop("'x' and 'g' must have the same length")
    g <- factor(g)
    k <- nlevels(g)
    if (k < 2L) 
...
The function in c has already been vectorized, as this seemed the best option to improve runtime. 
